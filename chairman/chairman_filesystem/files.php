...
//echo '<a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/files.php?id='.$id.'"><img id="back" src="'.$CFG->wwwroot.'/mod/chairman/pix/up.png" title="'.get_string('goback','chairman').'"></a>';
...
if($PRIVATE==1) {
    ...
    echo '<a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/add_folder.php?id='.$id.'&file='.$file.'"><img src="'.$CFG->wwwroot.'/mod/chairman/pix/switch_plus.gif'.'">'.get_string('addfolder', 'chairman').'</a><br/>';
    echo '<a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/add_file.php?id='.$id.'&file='.$file.'"><img src="'.$CFG->wwwroot.'/mod/chairman/pix/switch_plus.gif'.'">'.get_string('addfile', 'chairman').'</a>';
    }
    echo '</div>';
}
...
function print_folder($fold) {
    ...
    echo '<a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/files.php?id='.$id.'&file='.$fold->id.'"><img id="icon" src="'.$CFG->wwwroot.'/mod/chairman/pix/folder.png"></a>';
    echo '</td>';
    echo '<td>';
    echo '<b>'.$fold->name.'</b>';
    if($PRIVATE==1) {
        if (chairman_isadmin($id)){
        echo ' - <a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/delete_folder_script.php?id='.$id.'&fileid='.$fold->id.'" onClick="return confirm(\''.get_string('deletefolderquestion', 'chairman').'\');"><img src="'.$CFG->wwwroot.'/mod/chairman/pix/delete.gif"></a>';
        echo '<a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/add_folder.php?id='.$id.'&fileid='.$fold->id.'"><img src="'.$CFG->wwwroot.'/mod/chairman/pix/edit.gif"></a>';
        }
    ...
    echo '<br/><br/>';
    //echo '<a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/files.php?id='.$id.'&file='.$fold->id.'">'.get_string('openfolder','chairman').'</a>';
    echo '</td></tr></table>';
    echo '</div>';
}
...
function print_file($file) {
    ...
    if($PRIVATE==1) {
        if (chairman_isadmin($id)){
        echo ' - <a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/delete_file_script.php?id='.$id.'&file_id='.$file->id.'&file='.$file->parent.'" onClick="return confirm(\''.get_string('deletefilequestion', 'chairman').'\');"><img src="'.$CFG->wwwroot.'/mod/chairman/pix/delete.gif"></a>';
        echo '<a href="'.$CFG->wwwroot.'/mod/chairman/chairman_filesystem/add_file.php?id='.$id.'&fileid='.$file->id.'"><img src="'.$CFG->wwwroot.'/mod/chairman/pix/edit.gif"></a>';
        }
        echo '<br/>';
        if($file->private==0) {
            echo '('.get_string('public','chairman').')';
