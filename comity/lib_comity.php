...
function comity_printnavbar($id) {
    ...
    $comity = $DB->get_record('comity', array('id' => $mod->instance));
	echo '<div id="comity_menu" style="font-size: 14px">';
    ...
    echo '</td></tr>';
    if ($comity->use_forum == 1) {
        echo '<tr><td>';
        echo '<a href="'.$CFG->wwwroot.'/mod/forum/view.php?id='.$comity->forum.'" target="_blank">'.get_string('menu_forum', 'comity').'</a>';
        echo '</td></tr>'; 
    }
    if ($comity->use_wiki == 1) {
        echo '<tr><td>';
        echo '<a href="'.$CFG->wwwroot.'/mod/wiki/view.php?id='.$comity->wiki.'" target="_blank">'.get_string('menu_wiki', 'comity').'</a>';
        echo '</td></tr>'; 
    }
    if ($comity->use_questionnaire == 1) {
        echo '<tr><td>';
        echo '<a href="'.$CFG->wwwroot.'/mod/questionnaire/view.php?id='.$comity->questionnaire.'" target="_blank">'.get_string('menu_questionnaire', 'comity').'</a>';
        echo '</td></tr>'; 
    }
    echo '</table>';
	echo '</div>';
...
function comity_check($id) {
    ...
    global $cm;
    //Set session logo
	if (!empty($comity->logo)){
		$SESSION->comity_logo = "$CFG->dirroot/mod/comity/img/logos/$comity->logo";
		
	} else {
		$SESSION->comity_logo = "$CFG->dirroot/mod/comity/img/blank.jpg";
	}
    //If not a member, get out
    if ($comity->secured == 1){
        if ((!comity_isMember($id)) AND (!comity_isadmin($id))) {
            redirect($CFG->wwwroot.'/course/view.php?id='.$course->id, get_string('not_member', 'mod_comity'), 10);
        }
    }
}
...
foreach($dates as $date) {
    ...
    $offset = comity_get_timezone_offset($planner->timezone,$region_tz);
    //Calculate offset
...
*    @return int;
*/
function comity_get_timezone_offset($remote_tz, $origin_tz = null) {
    if($origin_tz === null) {
...
function comity_convert_strdate_time($year, $day, $month, $hour, $minute){
    if ($minute < 10) {
        $minute = '0'.$minute;
    }
    if ($hour < 10) {
