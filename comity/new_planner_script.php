...
if ($notify_now == 1){
    $president = $DB->get_record('comity_members', array('comity_id' => $id, 'role_id' => 1));
    //email information
    $from = $DB->get_record('user', array('id' => $president->user_id));
    $subject = get_string('plannernotificationsubject', 'comity');
	$subject = str_replace('{c}', $planner->name, $subject);
    $emailmessage = get_string('plannernotificationmessage', 'comity');
	$emailmessage = str_replace('{link}', "$CFG->wwwroot/mod/comity/planner.php?id=$id", $emailmessage);
