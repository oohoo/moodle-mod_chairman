...
print '<span style="display:inline-block; vertical-align:top; text-align: center;">'.get_string('save_pdf','comity').":<br>";
	...
	// if agenda object from database(called within view.php exists -- then we are updating)
        ...
        //If record exists then update location(redudent check, but added for clarity)
            ...
            if ($fromform->notify_now == 1){
                //get meeting
                $meeting = $DB->get_record('comity_events',array('id' => $event_id));
                //get president
                $president = $DB->get_record('comity_members', array('comity_id' => $comity_id, 'role_id' => 1));
                //email information
                $from = $DB->get_record('user', array('id' => $president->user_id));
                $subject = get_string('notify_reminder', 'comity').$meeting->summary;
                $emailmessage = get_string('notify_week_message', 'comity').'<p>'.$meeting->description.'</p>';
                //get all member emails.
                $members = $DB->get_records('comity_members', array('comity_id' => $comity_id));
